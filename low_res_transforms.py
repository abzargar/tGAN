...
class Resize(torch.nn.Module):
    ...
    def forward(self,img_list, mask_list):
        ...
        for img, mask in zip(img_list, mask_list):
            ...
            cleaned_mask = self.remove_small_objects(resized_mask_np, min_size=3)
            # Convert cleaned_mask back to PIL Image
            cleaned_mask_img = Image.fromarray(np.uint8(cleaned_mask))
            new_img_list.append(resized_img)
            new_mask_list.append(cleaned_mask_img)
